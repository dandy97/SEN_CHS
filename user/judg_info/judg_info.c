      ...
      __HAL_UART_GET_IT_SOURCE(&huart3, UART_IT_IDLE))
    {
      ...
      CLEAR_BIT(huart3.Instance->SR, USART_SR_IDLE);
     	...
     	uint16_t temp = huart3.hdmarx->Instance->NDTR;  
			if((1024 - temp) >=1 && (1024 - temp) <=500)
				Judge_Receive(JudgeDataBuffer[0]);
			}
...
void Judge_Receive(uint8_t *pData)
{
	...
	//48位是帧头A5
	if((JudgeDataBuffer[0][54] << 8 | JudgeDataBuffer[0][53]) == 0x201)// && (Verify_CRC16_Check_Sum_1(JudgeDataBuffer, 15 + 9) == 1))//血量
	{
	...
	{	
		InfantryJudge.volt = JudgeDataBuffer[0][8] << 8 | JudgeDataBuffer[0][7];	//电压
		
		FT.U[3] = JudgeDataBuffer[0][14];
		FT.U[2] = JudgeDataBuffer[0][13];
		FT.U[1] = JudgeDataBuffer[0][12];
		FT.U[0] = JudgeDataBuffer[0][11];
		
		InfantryJudge.power = FT.FF;	
	...
	{		
		InfantryJudge.bullet_freq = JudgeDataBuffer[0][8];//射频
		FT.U[3] = JudgeDataBuffer[0][12];
		FT.U[2] = JudgeDataBuffer[0][11];
		FT.U[1] = JudgeDataBuffer[0][10];
		FT.U[0] = JudgeDataBuffer[0][9];
		
		InfantryJudge.bullet_speed = FT.FF;	 //射速
	}
}
